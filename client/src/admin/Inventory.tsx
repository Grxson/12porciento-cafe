  ...
  return (
    <div className="p-6 sm:p-8">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <button onClick={loadOverview} className="flex items-center gap-2 text-xs text-coffee-600 dark:text-coffee-400 hover:text-coffee-900 dark:hover:text-cream border border-coffee-300 dark:border-coffee-700 px-3 py-2 transition-colors">
        ...
        {([
          ...
          <button key={id} onClick={() => setTab(id)}
            className={`px-4 py-2.5 text-sm border-b-2 -mb-px transition-colors ${
              tab === id ? 'border-gold-500 text-gold-500' : 'border-transparent text-coffee-600 dark:text-coffee-400 hover:text-coffee-900 dark:hover:text-cream'
            }`}>
            {label}
          </button>
        ))}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filteredProducts.map((p) => {
                      ...
                      return (
                        <tr key={p.id} className="border-b border-coffee-200/50 dark:border-coffee-800/50 hover:bg-coffee-100 dark:hover:bg-coffee-800/20 transition-colors">
                          ...
                          <td className="px-4 py-3 hidden sm:table-cell">
                            <span className="text-xs border border-coffee-300 dark:border-coffee-700 text-coffee-600 dark:text-coffee-400 px-2 py-0.5">{p.category}</span>
                          </td>
                          <td className="px-4 py-3">
                    ...
                    {filteredProducts.length === 0 && (
                      <tr><td colSpan={7} className="text-center py-12 text-coffee-500 dark:text-coffee-400">Sin productos</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {tab === 'movements' && (
        <div>
          ...
          {movLoading ? (
            ...
            <>
              <div className="text-coffee-500 dark:text-coffee-400 text-xs mb-3">{movTotal} movimientos</div>
            <div className="bg-coffee-100 dark:bg-coffee-900 border border-coffee-200 dark:border-coffee-800 overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                    ...
                    <tbody>
                      {movements.map((m) => {
                        ...
                        const Icon = mt.icon;
                        return (
                          <tr key={m.id} className="border-b border-coffee-200/50 dark:border-coffee-800/50 hover:bg-coffee-100 dark:hover:bg-coffee-800/20 transition-colors">
                            ...
                            <td className="px-4 py-3">
                              {m.product && (
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-coffee-900 dark:text-cream text-xs truncate max-w-[120px]">{m.product.name}</span>
                                </div>
                              )}
                            </td>
                            <td className="px-4 py-3">
                      ...
                      {movements.length === 0 && (
                        <tr><td colSpan={6} className="text-center py-12 text-coffee-500 dark:text-coffee-400">Sin movimientos</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                ...
                {([
                  ...
                  <button key={value} type="button"
                    ...
                    className={`flex items-start gap-2.5 p-3 border text-left transition-all ${
                      adjType === value ? 'border-gold-500/60 bg-gold-500/5' : 'border-coffee-300 dark:border-coffee-700 hover:border-coffee-400 dark:hover:border-coffee-600'
                    }`}>
        ...
        !alerts ? (
          ...
          <div className="space-y-6">
                  ...
                  {alerts.lowStock.map((p: any) => (
                    <div key={p.id} className="flex items-center justify-between bg-yellow-100 dark:bg-yellow-900/10 border border-yellow-300 dark:border-yellow-500/20 p-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-coffee-900 dark:text-cream text-sm">{p.name}</p>
                          {p.supplier && <p className="text-coffee-500 dark:text-coffee-400 text-xs">Proveedor: {p.supplier}</p>}
                        </div>
                      </div>
            ...
            {alerts.summary.outOfStockCount === 0 && alerts.summary.lowStockCount === 0 &&
              ...
              <p className="text-center text-coffee-500 dark:text-coffee-400 text-sm py-12">Sin alertas activas ✓</p>
            )}
          </div>
        )
      )}
...
function ThresholdEditor({ productId, current, onSaved }: { productId: string; current: number; onSaved: () => void }) {
  ...
  if (!editing) return (
    <button onClick={() => setEditing(true)} className="text-coffee-600 dark:text-coffee-400 hover:text-gold-500 text-xs transition-colors">
      {current} uds.
    </button>
  );
