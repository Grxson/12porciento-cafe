...
export default function CoffeePicker({ plan, selected, onChange }: Props) {
  ...
  useEffect(() => {
    productsApi
      ...
      .catch((err) => {
        console.error(err);
        setError('Error al cargar opciones de café.');
      })
  ...
  return (
    <div>
      <div className="mb-8 border-l-2 border-gold-500 bg-gold-500/10 px-4 py-3">
        <p className="text-xs font-semibold uppercase tracking-widest text-coffee-900 dark:text-cream">
          Todos nuestros cafés se entregan en grano entero
        </p>
        <p className="mt-1 text-xs text-coffee-600 dark:text-coffee-400">
          Moler justo antes de preparar conserva mejor aroma y sabor.
        </p>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div
          className={`text-xs font-semibold px-3 py-1 ${isReady ? 'bg-green-900/30 text-green-400' : 'bg-coffee-100 dark:bg-coffee-800 text-coffee-700 dark:text-coffee-300'}`}
        >
      ...
      <div className="flex gap-2 mb-8">
        {Array.from({ length: Math.min(slots.max, 10) }).map((_, i) => (
          <div
            key={i}
            className={`h-1.5 flex-1 max-w-[60px] transition-all duration-300 ${i < selected.length ? 'bg-gold-500' : 'bg-coffee-300 dark:bg-coffee-800'}`}
          />
        ))}
      </div>
            ...
            {products
              ...
              .map((product) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`relative aspect-[3/4] overflow-hidden border-2 transition-all ${isSelected ? 'border-gold-500' : 'border-transparent'}`}
                    >
                      <img
                      ...
                      />
                      <div
                        className={`absolute inset-0 transition-opacity duration-200 ${isSelected ? 'bg-coffee-950/30' : 'bg-gradient-to-t from-coffee-950/70 via-transparent to-transparent'}`}
                      />
                    ...
                    <div className="pt-2.5 pb-1">
                      <p className="text-xs text-gold-600 uppercase tracking-widest mb-0.5 truncate">
                        {product.region}
                      </p>
                      <p
                        className={`font-serif text-sm leading-tight transition-colors ${isSelected ? 'text-gold-500' : 'text-coffee-900 dark:text-cream'}`}
                      >
                        {product.name}
                      </p>
      ...
      {!isReady && selected.length > 0 && (
        ...
        >
          Selecciona {slots.min - selected.length} café
          {slots.min - selected.length !== 1 ? 's' : ''} más para continuar
        </motion.p>
      )}
    </div>
  );
