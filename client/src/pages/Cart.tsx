...
export default function Cart() {
  ...
  if (count() === 0) {
    return (
      <div className="min-h-screen pt-20 flex flex-col items-center justify-center gap-6 px-4 bg-coffee-50">
        <div className="w-24 h-24 border border-coffee-200 flex items-center justify-center">
          <ShoppingBag className="w-10 h-10 text-coffee-300" />
        </div>
        <div className="text-center">
          <h2 className="font-serif text-3xl text-coffee-900 mb-2">Carrito vacío</h2>
    ...
    <div className="min-h-screen pt-20 pb-24 md:pb-0 bg-coffee-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-10">
          ...
          <h1 className="font-serif text-4xl md:text-5xl text-coffee-900">
            Carrito
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <AnimatePresence initial={false}>
              {items.map(({ product, quantity }) => (
                <motion.div
                  ...
                  className="bg-white border border-coffee-200 hover:border-coffee-300 transition-colors p-4 flex gap-4"
                >
                  {/* Product image */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {product.weight && (
                            <span className="flex items-center gap-1 text-coffee-400 text-[10px] border border-coffee-200 px-1.5 py-0.5">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center border border-coffee-200">
                        <button
                        ...
                        <span className="w-10 text-center text-coffee-900 text-sm font-medium">{quantity}</span>
                        <button
                          onClick={() => updateQuantity(product.id, quantity + 1)}
                      ...
                      <p className="font-semibold text-coffee-900 text-lg">
                        ${(product.price * quantity).toLocaleString('es-MX')}
                        <span className="text-coffee-400 text-xs font-normal ml-1">MXN</span>
                      </p>
                    </div>
                  </div>
                </motion.div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-coffee-200 p-6 sticky top-24">
              ...
              <div className="space-y-2 mb-5 pb-5 border-b border-coffee-100">
                {items.map(({ product, quantity }) => (
                  <div key={product.id} className="flex justify-between text-sm">
                    <span className="text-coffee-600 truncate pr-2">{product.name} ×{quantity}</span>
                    <span className="text-coffee-900 shrink-0">${(product.price * quantity).toLocaleString('es-MX')}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex justify-between items-baseline mb-1">
                ...
                <span className="font-serif text-2xl text-coffee-900">${total().toLocaleString('es-MX')}</span>
              </div>
              ...
              <Link
                to="/tienda"
                className="block text-center text-coffee-500 hover:text-coffee-800 text-xs mt-4 transition-colors tracking-wide"
              >
                Seguir comprando
              </Link>
            </div>
