...
function AddCardForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        add(error.message ?? 'Error al guardar tarjeta', 'error');
        return;
      }
  ...
  const load = () => {
    ...
    usersApi
      ...
      .then((r) => {
        ...
        setDefaultId(r.data.defaultId);
      })
      .catch(() => {
        add('Error al cargar métodos de pago', 'error');
      })
      .finally(() => setLoading(false));
  };
    ...
    <div className="max-w-lg">
      ...
      {methods.length > 0 && (
        <div className="space-y-3 mb-6">
          {methods.map((pm) => (
            <div
              key={pm.id}
              className={`flex flex-wrap items-center justify-between gap-3 p-4 border transition-all ${
                defaultId === pm.id
      ...
      {methods.length === 0 && !adding && (
        <div className="text-center py-10 bg-white dark:bg-coffee-900 border border-coffee-200 dark:border-coffee-800 mb-6">
          ...
          <p className="text-coffee-600 dark:text-coffee-400 text-sm mb-1">
            Sin tarjetas guardadas
          </p>
          <p className="text-coffee-500 dark:text-coffee-600 text-xs">
            Agrega una para pagar más rápido.
          </p>
        </div>
      )}
      ...
      {adding ? (
        ...
        >
          <h3 className="font-serif text-lg text-coffee-900 dark:text-cream mb-5">
