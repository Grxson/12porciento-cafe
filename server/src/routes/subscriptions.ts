  ...
  try {
    ...
    if (existing && (existing.status === 'ACTIVE' || existing.status === 'PAUSED')) {
      if (existing.userId && existing.userId === userId) {
        ...
        res
          .status(409)
          .json({ error: 'Ya tienes una suscripción activa o pausada con este email' });
        return;
      } else {
    ...
    if (stripeCustomerId) {
      try {
        const priceInCents = Math.round(
          productRecords.reduce((sum: number, p: { price: number }) => sum + Number(p.price), 0) *
            100,
        );
        if (priceInCents < 100) {
...
// PUT /:id/admin — admin: edit plan, frequency, and coffee items
router.put('/:id/admin', requireAuth, async (req: AuthRequest, res: Response) => {
  async function syncStripeSubscription(
    subId: string,
    _newPlan: string | undefined,
    _newFrequency: string | undefined,
    _newItems: string[] | undefined,
  ) {
  ...
  try {
    const { plan, frequency, items } = req.body as {
      plan?: string;
      frequency?: string;
      items?: string[];
    };
    ...
    if (items !== undefined) {
      ...
      const currentItemCount = await prisma.subscriptionItem.count({
    ...
    const updateData: Prisma.SubscriptionUpdateInput = { grindPreference: 'GRANO' };
...
router.post('/b2b-inquiry', async (req: Request, res: Response) => {
  try {
    ...
    if (
      !contactoTelefono ||
      typeof contactoTelefono !== 'string' ||
    ...
    if (
      !volumenEstimado ||
      typeof volumenEstimado !== 'string' ||
      volumenEstimado.trim().length === 0
    ) {
    ...
    res
      .status(201)
      .json({ message: "Inquiry received. We'll contact you within 24 hours.", id: inquiry.id });
  } catch (err) {
    console.error(err);
